...
fn main() -> std::io::Result<()> {
    ...
    let mut v_sum = vec![0; boards.len()];
    let mut last_win_sum = 0;
    let mut all_win = 0;
    for pred in predictions {
        boards.iter_mut().for_each(|elem| elem.set_num(pred));
        for (i, game) in boards.iter().enumerate() {
            if let Some(sum) = game.check_on_win() {
                if v_sum[i] == 0 {
                    v_sum[i] = pred * sum;
                    last_win_sum = pred * sum;
                }
                all_win += 1;
            }
        }
        if all_win == boards.len() {
            break;
        } else {
            all_win = 0;
        }
